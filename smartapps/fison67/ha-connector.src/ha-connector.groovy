...
def haTypePage() {
    dynamicPage(name: "haTypePage", title: "Select a type", nextPage: "mainPage") {
       section("Configure HA API"){
           input "haAddType", "enum", title: "type", required: true, options: ["Default Sensor", "Switch", "Color Light", "White Light", "Motion Sensor", "Power Meter", "Illuminance Sensor", "Door Sensor", "Presence Sensor", "Temperature Sensor", "Humidity Sensor", "Battery", "Vacuum", "Blind", "Air Conditioner", "Button", "Fan", "Heater", "Naver Weather"], defaultValue: "Default"
       }
    }
}
    ...
    state.dataList.each { 
        ...
        if(friendly_name == null){
           ...
           if(!addedDNIList.contains("ha-connector-" + entity_id)){
            if(entity_id.contains("person.") || entity_id.contains("light.") || entity_id.contains("switch.") || entity_id.contains("fan.") || entity_id.contains("cover.") || entity_id.contains("sensor.") || entity_id.contains("vacuum.") || entity_id.contains("device_tracker.") || entity_id.contains("climate.") || entity_id.contains("media_player.")){
